...
def delete_messages(event: Event) -> str:
    ...
    if msg_ids == None or msg_ids == []:
        utils.new_message(event.api, event.chat.peer_id, message="❗ Ошибка дежурного, я не смог найти смс")
        return "ok"
    ...
    try:
        ...
        utils.new_message(event.api, event.chat.peer_id, message=f"❗ Произошла неизвестная ошибка.")
    return "ok"
